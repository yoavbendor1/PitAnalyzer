function s = PitAnalyzer_simplePolygonOrientation(V)
...
if idx == 1
    s = PitAnalyzer_vertexOrientation(V(n,:), V(1,:), V(2,:));
elseif idx == n
    s = PitAnalyzer_vertexOrientation(V(n-1,:), V(n,:), V(1,:));
else
    s = PitAnalyzer_vertexOrientation(V(idx-1,:), V(idx,:), V(idx+1,:));
end
end